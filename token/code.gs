...
/* ── TOKEN COUNTERS (original working system) ── */
...
/* ══════════════════════════════════════════════════════════════════
   ── NEW QUEUE SYSTEM ──
   Sheet "Tokens" columns (0-indexed):
     0  TokenID      e.g. "R-07"
     1  Dept         e.g. "REG"
     2  Prefix       e.g. "R"
     3  ParentName
     ...
     5  Phone
     6  Reason
     7  Status       WAITING | PRIORITY | CALLED | DONE | SKIPPED
     8  IssuedAt     ISO timestamp string
     9  CalledAt     ISO timestamp string (filled when CALLED)
    10  WaitMins     (calculated, stored for reference)
    11  ChildInfo    (unused, reserved)
   ══════════════════════════════════════════════════════════════════ */

// Dept config: key → {prefix, name}
var DEPT_CONFIG = {
  REG: { prefix: "R",  name: "Registrar"     },
  TRP: { prefix: "TR", name: "Transport"      },
  TC:  { prefix: "TC", name: "TC Incharge"    },
  AD:  { prefix: "AD", name: "Asst Director"  },
  FIN: { prefix: "F",  name: "Finance"        },
};

function ensureTokensSheet() {
  const wb = ss();
  if (!wb.getSheetByName("Tokens")) {
    const s = wb.insertSheet("Tokens");
    s.appendRow(["TokenID","Dept","Prefix","ParentName","AdmNo","Phone","Reason","Status","IssuedAt","CalledAt","WaitMins","ChildInfo"]);
    s.setFrozenRows(1);
  }
}

/* ── issueToken ──
   Counter lives in PropertiesService (script properties), keyed as DEPT_DATE.
   This is a simple string store with no sheet caching issues whatsoever. */
function issueToken(e) {
  ensureTokensSheet();
  const dept       = (e.parameter.dept || "").toUpperCase();
  const parentName = e.parameter.parentName || "";
  const admNo      = e.parameter.admNo || "";
  const phone      = e.parameter.phone || "";
  const reason     = e.parameter.reason || "";

  if (!dept || !DEPT_CONFIG[dept]) return j({ error: "Invalid department: " + dept });
  ...
  try {
    ...
    const prefix = DEPT_CONFIG[dept].prefix;
    const propKey = "CTR_" + dept + "_" + today;   // e.g. "CTR_REG_2026-03-28"

    const props   = PropertiesService.getScriptProperties();
    const current = Number(props.getProperty(propKey) || "0");
    const nextNum = current + 1;
    props.setProperty(propKey, String(nextNum));    // atomic string write, no sheet cache

    const tokenId  = prefix + "-" + String(nextNum).padStart(2, "0");
    const issuedAt = new Date().toISOString();

    ss().getSheetByName("Tokens").appendRow(
      [tokenId, dept, prefix, parentName, admNo, phone, reason, "WAITING", issuedAt, "", "", ""]
    );
    auditLog("visitor", "ISSUE_TOKEN", tokenId + " [" + dept + "] " + parentName);
    return j({ success: true, token: tokenId, dept, issuedAt });
...
/* ── getQueue (called by all 4 HTML files to read the queue) ──
   Returns all rows for a dept (or all depts if no dept param),
   filtered to today. No auth required — public read. */
function getQueue(e) {
  ensureTokensSheet();
  const dept  = (e.parameter.dept || "").toUpperCase();
  ...
  const rows = [];
  for (let i = 1; i < data.length; i++) {
    if (!String(data[i][8]).startsWith(today)) continue; // only today's rows
    if (dept && data[i][1] !== dept) continue;           // filter by dept if given
    rows.push(data[i]);
  }
...
/* ── callNext (called by staff-counter.html) ──
   Finds the next WAITING/PRIORITY token for the dept,
   marks it CALLED (and marks any existing CALLED token as DONE first). */
function callNext(e) {
  const u = authAny(e); if (!u) return j({ error: "Unauthorized" });
  ensureTokensSheet();
  const dept  = (e.parameter.dept || "").toUpperCase();
  if (!dept)  return j({ error: "dept required" });
  ...
  // Collect today's rows for this dept (with their sheet row index)
  const todayRows = [];
  for (let i = 1; i < data.length; i++) {
    if (data[i][1] === dept && String(data[i][8]).startsWith(today)) {
      todayRows.push({ idx: i, row: data[i] });
    }
  }

  // Auto-complete any currently CALLED token
  for (const { idx, row } of todayRows) {
    if (row[7] === "CALLED") {
      sheet.getRange(idx + 1, 8).setValue("DONE");
    }
  }

  // Find next PRIORITY first, then WAITING (sorted by IssuedAt)
  const waiting = todayRows
    .filter(r => r.row[7] === "WAITING" || r.row[7] === "PRIORITY")
    .sort((a, b) => {
      if (a.row[7] === "PRIORITY" && b.row[7] !== "PRIORITY") return -1;
      if (b.row[7] === "PRIORITY" && a.row[7] !== "PRIORITY") return 1;
      return new Date(a.row[8]) - new Date(b.row[8]);
    });

  if (waiting.length === 0) return j({ error: "Queue is empty" });

  const next = waiting[0];
  sheet.getRange(next.idx + 1, 8).setValue("CALLED");
  sheet.getRange(next.idx + 1, 10).setValue(now);
  auditLog(u.email, "CALL_NEXT", next.row[0] + " [" + dept + "]");
  return j({ success: true, token: next.row[0] });
}

/* ── updateTokenStatus (called by staff-counter.html for Done/Skip) ──
   Sets status for a specific TokenID. Requires login token. */
function updateTokenStatus(e) {
  const u = authAny(e); if (!u) return j({ error: "Unauthorized" });
  ensureTokensSheet();
  const tokenId = e.parameter.tokenId || "";
  const status  = (e.parameter.status || "").toUpperCase();

  if (!tokenId) return j({ error: "tokenId required" });
  if (!["WAITING","PRIORITY","CALLED","DONE","SKIPPED"].includes(status))
    return j({ error: "Invalid status: " + status });
  ...
  for (let i = 1; i < data.length; i++) {
    if (data[i][0] === tokenId) {
      ...
      if (status === "CALLED") sheet.getRange(i + 1, 10).setValue(new Date().toISOString());
      auditLog(u.email, "UPDATE_TOKEN", tokenId + " → " + status);
      return j({ success: true });
    }
  }
  return j({ error: "Token not found: " + tokenId });
}
...
function createTicket(e) {
  ...
  try {
    ...
    MailApp.sendEmail({to:ADMIN_EMAIL_CFG,name:"ZKTECO",subject:"🎫 "+tid+" ["+pri+"] "+title,body:title+"\n\n"+desc,htmlBody:html});
...
function terminalCommand(e) {
  ...
  const TBL={users:"Users",tickets:"Tickets",roles:"Roles",counters:"Counters",audit:"AuditLog",counterhistory:"CounterHistory",tokens:"Tokens"};
  if(cmd==="help") return j(["Commands:","  help  whoami  clear","  show tables  show storage  show audit","  describe <table>","  select * from users/tickets/roles/counters/tokens/audit","  select * from users where role=Admin","  count users/tickets/counters/tokens","  find user <email>"].join("\n"));
...
function getCountersPublic(e) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (!data[i][4]) {
      const p = String.fromCharCode(64 + i);
...
function setupTokensSheet() {
  ensureTokensSheet();
  Logger.log("Tokens sheet ready: " + ss().getSheetByName("Tokens").getDataRange().getValues()[0].join(", "));
}
